...
#      index1<-subject[index]
    ...
    if (plotid == 3)
    {
      ...
      quant.chisq <- qqPlot2(dm, distribution = 'chisq', df = q, pch = 20, 
                             cex = 1.2, cex.lab = 1.5, cex.axis = 1.3, main = "Gráfico 14: QQ-pLot",
                             ylab = expression(paste("Quantis da distância de Mahalanobis")),
                             xlab = "Quantis qui-quadrado")
    }
    
    if (plotid == 4)
    {
      par(mfrow = c(1,1), mar = c(12, 5, 1, 2))
      plot(dm, ylab = expression(paste("Distância de Mahalanobis")), 
           xlab = "Índice", main="Gráfico 10: Distância de Mahalanobis pelo índice",
  ...
  #    abline(h = LS2dm, lty = 3) #2*mean(dmp)
    ...
    if (plotid == 5)
    {
      ...
      plot(predi, rescp, xlab = expression(paste("Valores preditos")), 
           cex = 1.2, cex.lab = 1.3, cex.axis = 1.3, main="Gráfico 11: Resíduos x Preditos",
           ylab = expression(paste("Resíduos condicionais")), 
      ...
      hist(rescp, freq = F,breaks = c(-7:7), main = "Gráfico 12: Histograma", ylab="Densidade",
           xlab = expression(paste("Resíduos condicionais")), 
    ...
    if (plotid == 6)
    {
      ...
      qqPlot2(resmcp, ylab = "Resíduo minimamente confundido", main="Gráfico 8: QQ-Plot",
              xlab = "N(0,1) quantis", pch = 20, cex = 1.2, cex.lab = 1.2,
              cex.axis = 1.3)      
      hist(resmcp, freq = F,breaks = c(-6:6), 
           xlab = "Resíduo minimamente confundido", ylab = "Densidade",
           main = "Gráfico 9: Histograma", cex = 1.0, cex.lab = 1.2, cex.axis = 1.2, pch = 20)
    }
    
    if (plotid == 7)
    {
      par(mfrow = c(1,1))
      plot(Dccond,pch = 16, xlab = "Índice", main="Gráfico 13: Distância de Cook",
           ylab = "Distância de Cook condicional", cex = 1.2, cex.lab = 1.5,
           cex.axis = 1.3)
